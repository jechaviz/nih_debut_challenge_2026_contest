# Tarea: llevar NIH DEBUT Challenge 2026 a prod 100

Fecha de creacion: 2026-05-30
Deadline externo: 2026-06-05 11:59 PM EDT
Track interno: `student_partner_track`
Producto: `RenalCue: Community CKD Triage Software`

## Objetivo

Entregar un paquete listo para que un equipo undergraduate elegible registre y
suba la candidatura a NIH DEBUT Challenge 2026: necesidad clinica, equipo
requerido, software biomedical MVP, synthetic data pipeline, test cases,
checklist, submission packet, riesgos y automatizacion de draft/preflight con
ejecucion guiada para el portal oficial.

## Fuentes verificadas

- NIH DEBUT 2026: https://www.nih.gov/challenges/2026-design-biomedical-undergraduate-teams-debut-challenge
- VentureWell DEBUT: https://venturewell.org/debut/
- VentureWell guidelines: https://venturewell.org/debut-guidelines/
- NIDDK UACR/GFR: https://www.niddk.nih.gov/health-information/professionals/advanced-search/quick-reference-uacr-gfr
- CDC CKD basics: https://www.cdc.gov/kidney-disease/about/index.html
- KDIGO 2024 CKD guideline: https://kdigo.org/wp-content/uploads/2024/03/KDIGO-2024-CKD-Guideline.pdf
- FDA CDS guidance: https://www.fda.gov/regulatory-information/search-fda-guidance-documents/clinical-decision-support-software

## Alcance prod 100

- Official requirements, prize fit and deadlines captured.
- 3-8 undergraduate team requirement documented.
- Clinical need and evidence basis documented for CKD triage.
- Vlang core in `C:\git\v_projects\nih_debut_challenge_2026`.
- Vue3 CDN + SFC + UnoCSS demo in `C:\git\websites\nih_debut_challenge_2026`.
- Synthetic cases, case-results JSON, readiness JSON and manifest generated.
- Draft 6-page NIH narrative, optional VentureWell appendix, video outline,
  team statement, ethics/regulatory note and submission payload prepared.
- WAIBA registration and dashboard submission playbooks prepared for draft,
  preflight and official portal execution support.

## Estado actual

`ready_for_submission_packet`: artifacts are prepared locally for an eligible
undergraduate team captain to complete the official portal package with private
roster/signatures, faculty sponsor letter, final PDF and final video URL.

## Repos remotos

- Contest/evidence hub: https://github.com/jechaviz/nih_debut_challenge_2026_contest
- V product core: https://github.com/jechaviz/nih_debut_challenge_2026
- Vue web demo: https://github.com/jechaviz/nih_debut_challenge_2026_web

## Definition of done

- [x] Requisitos oficiales capturados.
- [x] Necesidad clinica definida.
- [x] Equipo undergraduate requerido documentado.
- [x] Biomedical software MVP Vlang creado.
- [x] Demo web Vue3 creado.
- [x] Test cases sinteticos creados.
- [x] Checklist y riesgos creados.
- [x] Submission packet creado.
- [x] Automation draft/preflight preparada.
- [x] V tests, package QA and WAIBA preflight receipts generated.
- [x] Desktop and mobile web render screenshots captured.
- [x] Competitive strategy, impact model and judge scorecard added.
- [x] Design controls and clinician validation protocol added.
- [x] Evidence table, competitive landscape and traceability matrix added.
- [ ] Equipo undergraduate real confirmado.
- [ ] Certification form firmado por todos los estudiantes.
- [ ] Faculty sponsor letter final agregada.
- [ ] PDF final y video final subidos por capitan autorizado.
